...
def validate_dimensions(
    width: int, height: int, is_emoji: bool = True, verbose: bool = True
) -> tuple[bool, dict]:
    """
    ...
    if is_emoji:
        ...
        if optimal:
            ...
            if verbose:
                print(
                    f"✗ {width}x{height} - emoji should be square, 128x128 recommended"
                )
            passes = False
    else:
...
def validate_gif(
    ...
    # Check file size
    size_pass, size_info = check_slack_size(gif_path, is_emoji, verbose=verbose)

    # Check dimensions
    try:
        with Image.open(gif_path) as img:
            width, height = img.size
            dim_pass, dim_info = validate_dimensions(
                width, height, is_emoji, verbose=verbose
            )
...
def is_slack_ready(
    ...
    """
    if verbose:
        passes, results = validate_gif(gif_path, is_emoji, verbose=True)
        if not passes:
        ...
        size_pass, _ = check_slack_size(gif_path, is_emoji, verbose=False)
        return size_pass
